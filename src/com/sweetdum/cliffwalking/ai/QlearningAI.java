...
/**
 * Created by Mengxiao Lin on 2016/12/19.
 */
public class QlearningAI implements ITrainablePlayer{
    private double q[][][];
    ...
    /**
     * Qlearning AI is an on-policy TD control algorithm.
     * Q(S,A) <- Q(S,A) + learnRate*(R + gamma*Q(S',A*) - Q(S,A)), A* = argmax Q(S',a)
     ...
     */
    public QlearningAI(double eps, double epsDecayRate, double gamma, double learnRate, int batchSize, Governor governor) {
    ...
    public void train() {
        ...
        while(true){
            ...
            while (!governor.isEpisodeEnd()){
                Action a1  = this.epsPlay(governor);
    ...
    private Action epsPlay(Governor governor){
